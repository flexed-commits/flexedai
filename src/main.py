...
from topgg import init_vote_db, init_contact_db, start_webhook_server, vote_reminder_loop, role_expiration_loop, check_and_assign_voter_role_on_join
import hashlib
import chess
import aiohttp
...
def get_discord_timestamp(dt=None, style='f'):
    """
    Convert datetime to Discord timestamp format.
    If dt is omitted or None, the current UTC time is used.
...
class TicTacToe(discord.ui.View):
        ...
        async def restart_callback(interaction: discord.Interaction):
            ...
            if self.is_ai:
                ...
                c.execute(
                    ...
                    (player1_id, player2_id, current_turn, channel_id, is_ai_game, difficulty) 
                    VALUES (?, ?, ?, ?, 1, ?)""",
                ...
                db_query(
                    ...
                    (str(interaction.message.id), new_game.game_id)
                )
            else:
            ...
            if self.is_ai:
                ...
                if is_draw:
                    ...
                    async def difficulty_callback(select_interaction: discord.Interaction):
                        ...
                        db_query(
                            "UPDATE tictactoe_games SET message_id = ? WHERE game_id = ?",
                            (str(select_interaction.message.id), new_game.game_id)
                        )
    ...
    async def forfeit(self, interaction: discord.Interaction):
        ...
        # Add New Game button after forfeit
        new_game_btn = discord.ui.Button(
            label='🎮 New Game',
            style=discord.ButtonStyle.success,
            custom_id=f'ttt_forfeit_new_{self.game_id}',
            row=4
        )
        new_game_btn.callback = self.create_new_game_callback(False)
        self.add_item(new_game_btn)
...
class DifficultySelect(discord.ui.Select):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        db_query(
            "UPDATE tictactoe_games SET message_id = ? WHERE game_id = ?",
            (str(interaction.message.id), game_id)
        )
...
class TicTacToeInvite(discord.ui.View):
    ...
    def __init__(self, challenger_id: str, opponent_id: str, channel_id: str):
        ...
        self.message = None
        self.challenger_mention = f"<@{challenger_id}>"
        self.opponent_mention = f"<@{opponent_id}>"
...
class ChessMoveModal(discord.ui.Modal):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        try:
            ...
            if game_over:
                ...
                if self.is_vs_ai:
                    ...
                    async def new_ai_game_callback(btn_interaction: discord.Interaction):
                        ...
                        existing_game = get_active_game(str(btn_interaction.user.id))
                        if existing_game:
                            await btn_interaction.response.send_message(
                                "❌ You already have an active chess game! Finish or resign from it first.",
                                ephemeral=True
                            )
                            return
                        ...
                        # Create new AI chess game directly
                        ...
                        c.execute('''INSERT INTO chess_games 
                                     ...
                                     VALUES (?, ?, ?, ?, ?)''',
                                 (str(btn_interaction.user.id), "stockfish", str(btn_interaction.user.id), new_board.fen(), str(btn_interaction.channel.id)))
                        new_game_id = c.lastrowid
                        conn.commit()
                        conn.close()
                        
                        board_image = await board_to_image(new_board)
                        new_file = discord.File(board_image, filename="chess_board.png")
                        
                        new_embed = discord.Embed(
                            title="♟️ Chess vs Stockfish",
                            description=f"**Player:** <@{btn_interaction.user.id}>\n**Opponent:** 🤖 Stockfish\n\n**Your turn!**",
                            color=discord.Color.blue()
                        )
                        new_embed.set_image(url="attachment://chess_board.png")
                        new_embed.set_footer(text=f"Game ID: {new_game_id}")
                        
                        new_view = ChessGameView(new_game_id, new_board, str(btn_interaction.user.id), True)
                        
                        await btn_interaction.followup.send(embed=new_embed, view=new_view, file=new_file)
            ...
            await interaction.followup.send(f"✅ Move played: `{move_str}`", ephemeral=True)
            
        except ValueError as e:
            await interaction.followup.send(f"❌ `{move_str}` is an invalid move. Please use standard chess notation (e.g., e4, Nf3, O-O).", ephemeral=True)
        except Exception as e:
...
class ChessGameView(discord.ui.View):
    ...
    async def resign_button(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        await interaction.followup.send("You have resigned from the game.", ephemeral=True)
...
class AIBot(commands.Bot):
    ...
    async def setup_hook(self):
        ...
        asyncio.create_task(start_webhook_server(self, port=24663))
...
async def on_guild_remove(guild):
    ...
    embed.add_field(name="📋 Server Name", value=guild.name, inline=True)
    embed.add_field(name="🆔 Server ID", value=f"`{guild.id}`", inline=True)
    embed.add_field(name="👑 Server Owner", value=f"{owner_mention} (`{owner_id}`)", inline=False)
    embed.add_field(name="👥 Member Count", value=guild.member_count, inline=True)
    ...
    embed.set_footer(text=f"Server Owner ID: {owner_id}")
    ...
    db_query(
        ...
        (f"Bot left server: {guild.name} (ID: {guild.id}, Owner: {owner_name} - {owner_id})",)
    )
...
**What this means:**
• You can use banned words without receiving strikes
...
async def report_user(ctx, member: discord.Member, *, reason: str, proof: discord.Attachment = None):
    """
    Report a user for misbehavior.
    Usage: /report @user <reason> [attach a file as proof]
    """
    if not ctx.guild:
        await ctx.send("❌ **This command can only be used in servers.**")
        return
    ...
    # Collect proof from the attachment parameter and any message attachments
    attachments_list = []
    if proof:
        attachments_list.append(proof.url)
    if ctx.message and ctx.message.attachments:
        for att in ctx.message.attachments:
            if att.url not in attachments_list:
                attachments_list.append(att.url)
    
    # Combine proof text and attachments
    full_proof = "\n".join(attachments_list) if attachments_list else "No proof attached"
...
async def start_mode(ctx):
    ...
    await ctx.send(f"Bot will now respond to all messages in {ctx.channel.mention}")
...
async def stop_mode(ctx):
    ...
    await ctx.send(f"Bot will now only respond when mentioned in {ctx.channel.mention}")
...
async def build_leaderboard_page(results, title, description, color, value_label, page, total_pages):
    """Build an embed for a leaderboard page."""
    ...
    if results:
        ...
        for i, row in enumerate(page_results):
            ...
            try:
                ...
                name = f"User {user_id}"
            medal = "🥇" if global_idx == 1 else "🥈" if global_idx == 2 else "🥉" if global_idx == 3 else f"**{global_idx}.**"
            leaderboard_text += f"{medal} {name} - **{value}** {value_label}\n"
...
def build_pagination_view(ctx_author_id, current_page, total_pages, on_page_change):
    """
    Build a View with pagination buttons.
    on_page_change is an async callback: on_page_change(interaction, new_page)
    Only added when total_pages > 1.
    """
    class GoToPageModal(discord.ui.Modal, title="Go To Page"):
        page_input = discord.ui.TextInput(
            label="Page Number",
            placeholder=f"Enter a number between 1 and {total_pages}",
            min_length=1,
            max_length=5
        )

        def __init__(self, author_id, cur_page, tot_pages, callback):
            super().__init__()
            self.author_id = author_id
            self.cur_page = cur_page
            self.tot_pages = tot_pages
            self.callback_fn = callback
            self.page_input.placeholder = f"Enter a number between 1 and {tot_pages}"
        ...
        async def on_submit(self, interaction: discord.Interaction):
            ...
            try:
                ...
                await interaction.response.send_message(
                    f"❌ Invalid input. Please type a number between **1** and **{self.tot_pages}**.",
                    ephemeral=True
                )
                return
            if new_page < 1 or new_page > self.tot_pages:
                await interaction.response.send_message(
                    f"❌ Page **{new_page}** does not exist. Please type a number between **1** and **{self.tot_pages}**.",
                    ephemeral=True
                )
                return
            await self.callback_fn(interaction, new_page - 1)
    ...
    class PaginationView(discord.ui.View):
        ...
        def _update_buttons(self):
            # Clear existing pagination buttons (keep row 0 and 1 items if any)
            # We'll store them separately and re-add
            pass

        @discord.ui.button(label="⏮ First", style=discord.ButtonStyle.secondary, row=2)
        async def first_page(self, interaction: discord.Interaction, button: discord.ui.Button):
            if interaction.user.id != ctx_author_id:
                await interaction.response.send_message("❌ This is not your leaderboard!", ephemeral=True)
                return
            await on_page_change(interaction, 0)
        ...
        async def prev_page(self, interaction: discord.Interaction, button: discord.ui.Button):
            if interaction.user.id != ctx_author_id:
                await interaction.response.send_message("❌ This is not your leaderboard!", ephemeral=True)
                return
            await on_page_change(interaction, current_page - 1)
        ...
        @discord.ui.button(label="Last ⏭", style=discord.ButtonStyle.secondary, row=2)
        async def last_page(self, interaction: discord.Interaction, button: discord.ui.Button):
            if interaction.user.id != ctx_author_id:
                await interaction.response.send_message("❌ This is not your leaderboard!", ephemeral=True)
                return
            await on_page_change(interaction, total_pages - 1)

        @discord.ui.button(label="🔢 Go To Page", style=discord.ButtonStyle.success, row=3)
        async def go_to_page(self, interaction: discord.Interaction, button: discord.ui.Button):
            if interaction.user.id != ctx_author_id:
                await interaction.response.send_message("❌ This is not your leaderboard!", ephemeral=True)
                return
            modal = GoToPageModal(ctx_author_id, current_page, total_pages, on_page_change)
            await interaction.response.send_modal(modal)
...
async def leaderboard(ctx, server_leaderboard: bool = True):
    ...
    # Server leaderboard requires a guild context
    if server_leaderboard and not ctx.guild:
        await ctx.send(
            "❌ Server leaderboards can only be used inside a server. "
            "Use `server_leaderboard: False` for the global leaderboard.",
            ephemeral=True
        )
        return
    ...
    # AI Chat Leaderboard
    async def show_ai_chat_leaderboard(interaction, is_server, dropdown_view, page=0):
        guild = interaction.guild

        if is_server and guild:
            query = '''
            ...
            '''
            results = db_query(query, (str(guild.id),), fetch=True)
            title = f"🏆 {guild.name} - AI Chat Leaderboard"
        else:
            query = '''
                ...
                ORDER BY total DESC
            '''
            results = db_query(query, fetch=True)
            title = "🌍 Global AI Chat Leaderboard"

        total_pages = max(1, -(-len(results) // ITEMS_PER_PAGE)) if results else 1
        page = max(0, min(page, total_pages - 1))

        embed = await build_leaderboard_page(
            results, title, "*All-time AI chat leaderboard*",
            discord.Color.blue(), "messages", page, total_pages
        )
        if not results:
            embed.description = "No data available yet. Start chatting with the AI!"

        if len(results) > ITEMS_PER_PAGE:
            async def on_page_change(inter, new_page):
                await show_ai_chat_leaderboard(inter, is_server, dropdown_view, new_page)

            pagination = build_pagination_view(ctx.author.id, page, total_pages, on_page_change)
            # Merge dropdown items into pagination view (row 0)
            combined_view = discord.ui.View(timeout=180)
            for item in dropdown_view.children:
                combined_view.add_item(item)
            for item in pagination.children:
                combined_view.add_item(item)
            view_to_use = combined_view
        else:
            view_to_use = dropdown_view

        if hasattr(interaction, '_responded') and interaction._responded:
    ...
    # Chess Leaderboard
    async def show_chess_leaderboard(interaction, is_server, dropdown_view, page=0):
        guild = interaction.guild
        ...
        if is_server and guild:
            ...
            '''
            results = db_query(query, (str(guild.id),), fetch=True)
            title = f"🏆 {guild.name} - Chess Leaderboard"
        else:
            query = '''
        ...
        embed = await build_leaderboard_page(
            results, title, "*All-time wins against Stockfish*",
            discord.Color.gold(), "wins", page, total_pages
        )
        if not results:
        ...
        if len(results) > ITEMS_PER_PAGE:
            async def on_page_change(inter, new_page):
                await show_chess_leaderboard(inter, is_server, dropdown_view, new_page)
    ...
    # Tic-Tac-Toe Difficulty Selection - TWO ROWS OF DROPDOWNS (rebuilt safely)
    async def show_tictactoe_difficulty_select(interaction, is_server, parent_view):
        ...
        # We rebuild the category select from scratch instead of stealing the
        # item from parent_view — moving an item out of its original view breaks
        # the parent's callback and causes "interaction failed" errors.
        class TicTacToeLeaderboardView(discord.ui.View):
            def __init__(self):
                super().__init__(timeout=None)

            # --- ROW 0: category select (rebuilt fresh) ---
            @discord.ui.select(
                placeholder="Choose a leaderboard category",
                row=0,
                options=[
            ...
            async def category_select(self, inter: discord.Interaction, select: discord.ui.Select):
                if inter.user.id != ctx.author.id:
                    await inter.response.send_message("❌ This is not your leaderboard!", ephemeral=True)
                    return
                category = select.values[0]
                if category == "ai_chat":
                    await show_ai_chat_leaderboard(inter, is_server, self)
                elif category == "chess":
                    await show_chess_leaderboard(inter, is_server, self)
                elif category == "tictactoe":
                    await show_tictactoe_difficulty_select(inter, is_server, self)

            # --- ROW 1: difficulty select ---
            @discord.ui.select(
                placeholder="Select difficulty level",
                row=1,
                options=[
                    discord.SelectOption(label="Easy", emoji="🟢", value="Easy"),
                    discord.SelectOption(label="Middle", emoji="🟡", value="Middle"),
                    discord.SelectOption(label="Hard", emoji="🔴", value="Hard"),
                    discord.SelectOption(label="Insane", emoji="💀", value="Insane")
                ]
            )
        ...
        new_view = TicTacToeLeaderboardView()
        await interaction.response.edit_message(embed=embed, view=new_view)
    
    # Tic-Tac-Toe Leaderboard by Difficulty - KEEPS BOTH DROPDOWNS VISIBLE
    async def show_tictactoe_leaderboard(interaction, is_server, difficulty, dropdown_view, page=0):
        guild = interaction.guild
        ...
        if is_server and guild:
            query = '''
                ...
                WHERE guild_id = ? AND difficulty = ?
                ORDER BY wins DESC
            '''
            results = db_query(query, (str(guild.id), difficulty), fetch=True)
            title = f"🏆 {guild.name} - Tic-Tac-Toe ({difficulty.capitalize()})"
        else:
            query = '''
                SELECT user_id, SUM(wins) as total
                FROM leaderboard_tictactoe
                ...
                ORDER BY total DESC
            '''
            results = db_query(query, (difficulty,), fetch=True)
        ...
        embed = await build_leaderboard_page(
            results, title, f"*All-time wins • Difficulty: {difficulty.capitalize()}*",
            discord.Color.purple(), "wins", page, total_pages
        )
        if not results:
        ...
        if len(results) > ITEMS_PER_PAGE:
            async def on_page_change(inter, new_page):
                await show_tictactoe_leaderboard(inter, is_server, difficulty, dropdown_view, new_page)
    ...
    # Patch TicTacToeLeaderboardView's difficulty_select to pass dropdown_view correctly
    # (It's handled inside show_tictactoe_difficulty_select — the view passes itself)
...
async def whoami(ctx):
    ...
    if is_blacklisted:
        embed.add_field(
            ...
            value=f"Your account is **suspended** from using the bot.\nContact the bot owner or [join the support server](<{os.getenv('SUPPORT_SERVER_INVITE', 'https://discord.com/invite/XMvPq7W5N4')}>) for appeals.",
            inline=False
        )
    elif strikes >= 2:
        embed.add_field(
...
async def on_message(message):
    ...
    # ── Word filter ──────────────────────────────────────────────────────────
    # Global banned_words  → strike system (guilds + DMs); bypassed by is_bypass_user()
    # Server censor words  → guild-only, no strike, just notify; bypassed by is_server_censor_bypass()
    # When a server has its own censor list the global list is IGNORED for that guild.

    async def _give_strike(member_id: str, guild_id_str: str, channel):
        """Issue one strike for using a global banned word, log it, DM the user."""
        uid = str(member_id)
        res = db_query("SELECT strikes FROM users WHERE user_id = ?", (uid,), fetch=True)
        current = res[0][0] if res else 0
        new_strikes = current + 1
        is_banned = 1 if new_strikes >= 3 else 0
        db_query(
            "INSERT OR REPLACE INTO users (user_id, strikes, blacklisted) VALUES (?, ?, ?)",
            (uid, new_strikes, is_banned)
        )
        log_msg = (
            f"Auto-strike issued to {uid} for using a banned word. "
            f"Total: {new_strikes}/3." +
            (f" | User AUTO-BLACKLISTED." if is_banned else "")
        )
        db_query("INSERT INTO admin_logs (log) VALUES (?)", (log_msg,))
        ...
        dm_msg = (
            f"⚡ **Strike Issued**\n\n"
            f"**Reason:** Using a globally banned word.\n"
            f"**Total Strikes:** {new_strikes}/3\n"
        )
        if is_banned:
            dm_msg += (
                f"\n🚫 **ACCOUNT SUSPENDED** — You have reached 3 strikes and have been "
                f"automatically blacklisted from {BOT_NAME} Bot.\n\n"
                f"**Appeal:** Contact the bot owner: <@!{OWNER_ID}>\n"
                f"**Support Server:** {os.getenv('SUPPORT_SERVER_INVITE', 'https://discord.com/invite/XMvPq7W5N4')}"
            )
        else:
            dm_msg += f"⚠️ **Warning:** {3 - new_strikes} strike(s) remaining before blacklist."
        dm_msg += f"\n\n*Timestamp: {get_discord_timestamp(style='F')}*"
        dm_sent = await send_user_dm(uid, dm_msg)

        # Log embed to strikes channel
        log_embed = discord.Embed(
            title="⚡ Auto-Strike (Banned Word)" if not is_banned else "🚫 User Auto-Blacklisted (3 Strikes)",
            color=discord.Color.orange() if not is_banned else discord.Color.dark_red(),
            timestamp=datetime.datetime.now(datetime.timezone.utc)
        )
        log_embed.add_field(name="👤 User ID", value=f"`{uid}`", inline=True)
        log_embed.add_field(name="📊 Total Strikes", value=f"{new_strikes}/3", inline=True)
        log_embed.add_field(name="📬 DM Sent", value="✅" if dm_sent else "❌", inline=True)
        log_embed.add_field(name="🏠 Guild", value=guild_id_str or "DM", inline=True)
        log_embed.add_field(name="📌 Status", value="🚫 AUTO-BANNED" if is_banned else f"⚠️ Active ({3 - new_strikes} remaining)", inline=True)
        await log_to_channel(bot, 'strikes', log_embed)
        await log_to_channel(bot, 'banned_words', log_embed)
        ...
        warn_text = (
            ...
            f"You have received a strike (**{new_strikes}/3**)."
        )
        if is_banned:
            warn_text += "\n🚫 You have been **blacklisted** from the bot."
        await channel.send(warn_text, delete_after=15)

    if message.guild:
        guild_id_str = str(message.guild.id)
        if has_server_banned_words(message.guild.id):
            # Server censor list active — only check server words, no strikes
            if not is_server_censor_bypass(message.guild.id, message.author.id):
                server_banned = [w.lower() for w in get_server_banned_words(message.guild.id)]
                if any(bw in content_low for bw in server_banned):
                    print(f"🔇 CENSOR: Message from {message.author.name} contained server censor word")
                    await message.channel.send(
                        f"⚠️ {message.author.mention}, your message contained a censored word (server filter).",
                        delete_after=10
                    )
        else:
            # No custom server censor list — use global banned words with strike system
            if not is_bypass_user(message.author.id):
                banned = db_query("SELECT word FROM banned_words", fetch=True)
                banned_lower = [bw[0].lower() for bw in banned] if banned else []
                if any(bw in content_low for bw in banned_lower):
                    print(f"⚡ STRIKE: Message from {message.author.name} contained global banned word")
                    await _give_strike(message.author.id, guild_id_str, message.channel)
    else:
        # DM — global filter only, with strike system
        if not is_bypass_user(message.author.id):
            banned = db_query("SELECT word FROM banned_words", fetch=True)
            banned_lower = [bw[0].lower() for bw in banned] if banned else []
            if any(bw in content_low for bw in banned_lower):
                print(f"⚡ STRIKE (DM): Message from {message.author.name} contained global banned word")
                await _give_strike(message.author.id, "DM", message.channel)
...
async def on_command_error(ctx, error):
    """Handle command errors - show user-friendly messages"""
    if isinstance(error, commands.CommandNotFound):
        # Extract the attempted command name
        command_name = ctx.message.content.split()[0].lstrip('/')
        await ctx.send(
            f"❌ `/{command_name}` command doesn't exist. Please check the commands list using `/help` for more info."
        )
    elif isinstance(error, commands.CheckFailure):
        pass  # Already handled in the individual checks
        ...
        # Re-raise other errors so they don't get silently swallowed
        raise error
...
async def on_app_command_error(interaction: discord.Interaction, error: app_commands.AppCommandError):
    ...
    if isinstance(error, app_commands.CommandNotFound):
        ...
        try:
            await interaction.response.send_message(
                f"❌ An error occurred while running this command. Please try again.",
                ephemeral=True
            )
        except discord.InteractionResponded:
            pass
